...
- **Infrastructure boilerplate**:
  ...
  - Override `__tablename__` in the `Base(DeclarativeBase)` class using `@declared_attr.directive` and a regular expression to robustly convert `PascalCase` class names to `snake_case` table names (e.g. `re.sub(r'(?<!^)(?=[A-Z])', '_', cls.__name__).lower()`).
...
- **Relationships**:
  - Define `relationship()` explicitly with `back_populates` using standard string literals (e.g., `back_populates="posts"`).
